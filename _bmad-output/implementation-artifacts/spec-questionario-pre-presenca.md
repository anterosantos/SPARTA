...
**Approach:** Novo toggle "Não vou estar presente" só na fase `pre` do `FatigueQuestionnaire` — mesmo padrão visual/interativo (checkbox-card) do toggle já usado em `/agenda/[sessionId]` (`absence-form.tsx`), a pedido explícito do utilizador para consistência visual. Reaproveita o mecanismo de ausência já existente (`declarePlayerAbsence`/`cancelPlayerAbsence`). Ao submeter o questionário pré: se o toggle estiver marcado, grava o questionário normalmente e depois chama `declarePlayerAbsence`; se estiver desmarcado (omissão), chama `cancelPlayerAbsence` (no-op se o jogador não estava ausente). O toggle vem pré-marcado (`initialAbsent`) se já existir uma ausência registada para a sessão (ex.: declarada antes pelo ecrã "Presença").
